...
test.describe('Wallet & Store: Purchases & Achievements Flow', () => {
  ...
  for (const locale of locales) {
    test(`[${locale}] Purchase item from Store and verify achievement`, async ({ page }) => {
      ...
      const points = Number.parseInt(pointsText.replace(/[^0-9]/g, ''));
      ...
      const price = Number.parseInt(itemPriceText.replace(/[^0-9]/g, ''));
      ...
      if (points >= price) {
        ...
        const newPoints = Number.parseInt(newPointsText.replace(/[^0-9]/g, ''));
