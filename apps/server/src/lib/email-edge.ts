// Edge-compatible email service using Resend API
// Uses fetch instead of process.env for Cloudflare Workers compatibility
...
export function createEdgeEmailService(env: EdgeEnv) {
  const resend = new Resend(env.RESEND_API_KEY);
  const baseUrl = env.APP_URL || 'https://hocchungkhoan.com.vn';
    ...
    async sendEnrollmentEmail({
      ...
      try {
        const { data, error } = await resend.emails.send({
          from: 'V-EdFinance <noreply@hocchungkhoan.com.vn>',
          to: [to],
...
export type EdgeEmailService = ReturnType<typeof createEdgeEmailService>;
