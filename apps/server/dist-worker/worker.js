...
var userRouter = router({
  ...
  testDb: publicProcedure.query(async ({ ctx }) => {
    try {
      const count = await ctx.db.select({ count: sql`count(*)` }).from(users);
...
function createEdgeDb(databaseUrl) {
  const sql4 = neon(databaseUrl);
  return drizzle(sql4, { schema: schema_exports });
}

// src/lib/auth-edge.ts
function createEdgeAuth(env) {
...
// src/trpc/context-edge.ts
async function createEdgeContext({ req }, env) {
  const db = createEdgeDb(env.DATABASE_URL);
...
app.on(["GET", "POST"], "/api/auth/**", async (c) => {
  try {
    ...
    return c.json({ error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : void 0 }, 500);
  }
});
