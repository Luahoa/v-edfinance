...
test.describe('Profile & Settings Management (E008)', () => {
  ...
  test('should upload avatar to R2', async ({ page }) => {
    ...
    const testImagePath = path.join(process.cwd(), 'apps', 'web', 'tests', 'fixtures', 'test-avatar.png');
  ...
  test('should handle investment profile updates', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="profile-analysis-complete"]')).toBeVisible({ timeout: 15000 });
  });
});
