  ...
  for (const locale of locales) {
    test(`should allow a new user to register in ${locale} with high latency`, async ({ page }) => {
      ...
      await page.route('**/api/**', async (route) => {
        ...
        if (url.includes('/auth/register')) {
          ...
          await route.fulfill({
            status: 200,
            contentType: 'application/json',
            body: JSON.stringify({ points: 1250, streak: 7 }),
          });
        } else {
          await route.fulfill({
            status: 200,
      ...
      const submitBtn = page
        .locator('button')
        .filter({ hasText: /注册|Register|Đăng ký/i })
        .first();
...
test.describe('The Holy Trinity: AI Interaction', () => {
  test('should allow user to chat with AI mentor', async ({ page }) => {
          ...
          body: JSON.stringify([
            {
              id: 'm1',
              role: 'ASSISTANT',
              content: 'Đây là câu trả lời từ AI Mentor.',
    ...
    const threadBtn = page
      .locator('button')
      .filter({ hasText: /Bắt đầu thảo luận|thảo luận|Chat/i })
      .first();
    await expect(threadBtn).toBeVisible({ timeout: 45000 });
    await threadBtn.click();

    // Use regex for content to handle possible hydration mismatches
    await expect(page.locator('text=/Đây là câu trả lời từ AI Mentor/')).toBeVisible({
      timeout: 30000,
    });
  });
});
  ...
  test('should show streak and points on dashboard', async ({ page }) => {
    await page.route('**/api/**', async (route) => {
      ...
      if (url.includes('/users/dashboard-stats')) {
        ...
        url.includes('/social/recommendations') ||
...
test.describe('The Holy Trinity: Conversion', () => {
  test('should allow user to complete a simulated purchase', async ({ page }) => {
    ...
    const buyButton = page
      .locator('button')
      .filter({ hasText: /Mua|Buy|购买/ })
      .first();
    await buyButton.waitFor({ state: 'visible', timeout: 45000 });
    await buyButton.click({ force: true });

    await expect(
      page.locator('text=thành công').or(page.locator('text=success')).or(page.locator('text=成功'))
