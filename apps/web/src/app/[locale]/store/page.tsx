...
import { ShoppingBag, Star, ShieldCheck, CheckCircle2, AlertCircle } from 'lucide-react';
    ...
    <div className="container mx-auto max-w-4xl p-6">
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 px-4 py-2 rounded-full flex items-center gap-2 border border-blue-100 dark:border-blue-800">
          ...
          <span className="font-bold text-blue-700 dark:text-blue-400">{userPoints.toLocaleString()} {td('points')}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {loading ? (
          Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="animate-pulse bg-white dark:bg-zinc-900 h-48 rounded-xl border border-zinc-200 dark:border-zinc-800" />
          ))
        ) : (
          items.map((it) => {
            const item = it as { id: string; name: Record<string, string>; description: Record<string, string>; price: number };
            return (
              <div key={item.id} className="bg-white dark:bg-zinc-900 rounded-xl p-6 shadow-sm border border-zinc-200 dark:border-zinc-800 flex flex-col justify-between hover:border-blue-300 dark:hover:border-blue-700 transition-colors">
                <div>
                  ...
                  <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">{item.name[locale] || item.name.en}</h3>
