...
async function validateMovePlan() {
  ...
  const categorization: FileCategory[] = JSON.parse(fs.readFileSync(categorizationPath, 'utf-8'));
  ...
  // 3. Count categories
  const categoryCounts = categorization.reduce(
    (acc, file) => {
  ...
  Object.entries(categoryCounts)
    ...
    .forEach(([category, count]) => {
      ...
      console.log(
        `  ${category.padEnd(12)} ${String(count).padStart(3)} files (${percent.padStart(5)}%)`
      );
    });
  ...
  const filesToMove = categorization.filter(
    (f) => f.category !== 'core' && f.category !== 'delete'
  );
