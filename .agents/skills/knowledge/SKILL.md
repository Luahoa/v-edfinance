...
## Tool Quick Reference (V-EdFinance Adaptation)

| Goal | Tool |
|------|------|
| Find threads | `find_thread query\|after:Xd\|file:path` (Amp builtin) |
| Read thread | `read_thread` with focused goal (Amp builtin) |
| Parallel extraction | `Task` (spawn multiple) |
| Find definitions | `finder` (semantic search) |
| Find references | `Grep` (text search in codebase) |
| Semantic search | `finder` |
| Area overview | `Read` directory listing |
| Synthesis | `oracle` |
| Read doc | `Read` |
| Search docs | `Grep` |
...
**Note**: V-EdFinance does not use GKG MCP server. Use `finder` for codebase exploration and `Grep` for exact text search.
